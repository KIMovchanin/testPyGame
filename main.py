...
while running:
    ...
    if gameplay:
        ...
        # условие и в наш список добавляется коллайдер за границами экрана. А в скрипте под этим комментарием мы это
        # считываем и если в списке что-то появляется, то мы перебираем это и рисуем на экране картинку по
        # координатам того колайдера, что мы добавляем в конце, затем изменяем x для этого элемента постоянно на - 10
        # и проверяем касается ли коллайдер игрока коллайдера зомби и в случае соприкосновения мы теряем хп
